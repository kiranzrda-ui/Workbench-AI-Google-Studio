  ...
  return (
    <div className="flex-1 overflow-y-auto bg-slate-50 p-8 space-y-10">
      <header className="flex justify-between items-end">
        <div>
          <h2 className="text-4xl font-black text-slate-900 tracking-tight leading-none uppercase">Collaboration & Governance</h2>
        ...
        <div className="flex bg-slate-100 p-1 rounded-2xl">
           {(['Monitor', 'Audit Ledger'] as const).map(tab => (
             <button 
               ...
               className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-white text-slate-900 shadow-xl' : 'text-slate-500 hover:text-slate-700'}`}
             >
               {tab}
             </button>
           ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
           <div className="lg:col-span-2 space-y-8">
              <section className="bg-white rounded-[40px] border border-slate-200 shadow-sm overflow-hidden p-8">
                 <h3 className="font-black text-slate-800 text-[10px] uppercase tracking-widest flex items-center gap-2 mb-6">
                    ...
                    {criticalModels.length > 0 ? criticalModels.map(m => (
                       <div key={m.id} className="flex items-center gap-6 p-6 bg-rose-50 border border-rose-100 rounded-[32px]">
                          ...
                          <div className="flex-1">
                             <div className="text-lg font-black text-rose-900 uppercase tracking-tight">{m.name}</div>
                             <p className="text-xs text-rose-700 mt-1 font-medium italic">Accuracy fell below 85% threshold. Retraining job suggested.</p>
                          </div>
                          <button className="px-6 py-3 bg-rose-900 text-white font-black rounded-2xl text-[10px] uppercase tracking-widest shadow-lg active:scale-95 transition-transform">Fix Now</button>
                       </div>
                    )) : (
              ...
              <div className="grid grid-cols-2 gap-8">
                 <div className="bg-indigo-600 rounded-[40px] p-8 text-white shadow-xl">
                    ...
                    <p className="text-xs opacity-80 font-medium leading-relaxed">System-wide adherence to fairness and bias constraints.</p>
                 </div>
                 <div className="bg-white rounded-[40px] p-8 border border-slate-200 shadow-sm">
                    ...
                    <p className="text-xs text-slate-500 font-medium leading-relaxed">Every model in production has a verified provenance trail.</p>
                 </div>
              </div>
           </div>
           ...
           <div className="bg-slate-900 rounded-[40px] p-8 text-white shadow-2xl overflow-y-auto">
              ...
              <div className="space-y-4">
                 {approvalQueue.map(req => (
                    <div key={req.id} className="bg-slate-800 p-6 rounded-[32px] border border-slate-700 hover:border-indigo-500 transition-colors">
                      <div className="text-sm font-black text-white uppercase tracking-tight">{req.modelName}</div>
                      <div className="text-[10px] text-slate-500 font-bold mt-2 uppercase">Requester: {req.requester} • {new Date(req.timestamp).toLocaleDateString()}</div>
                      <div className="flex gap-2 mt-6">
                         <button 
                           onClick={() => onApprove && onApprove(req.id)}
                           className="flex-1 py-3 bg-emerald-600 rounded-2xl font-black text-[10px] uppercase hover:bg-emerald-500 transition-colors shadow-lg shadow-emerald-900/50"
                         >
                           Approve
                         </button>
              ...
              <tbody className="divide-y divide-slate-50 font-mono text-[11px]">
                 {auditLogs.map(log => (
                   <tr key={log.id} className="hover:bg-slate-50 transition-colors group">
                      ...
                      <td className="px-6 py-5 font-black text-slate-700 uppercase tracking-tighter">{log.user}</td>
                      <td className="px-6 py-5 text-slate-900 font-black uppercase group-hover:text-indigo-600">{log.action}</td>
                      <td className="px-6 py-5 text-indigo-600 font-bold uppercase tracking-tight">{log.platform}</td>
                      <td className="px-6 py-5">
                         <span className="px-2 py-1 bg-emerald-50 text-emerald-600 rounded font-black text-[9px] uppercase border border-emerald-100">SHA-256 Valid</span>
                      </td>
                   </tr>
                 ))}
              </tbody>
