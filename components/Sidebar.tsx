  ...
  return (
    <aside className="w-64 border-r border-slate-200 bg-white flex flex-col p-6 hidden md:flex shrink-0">
      ...
      <div className="flex-1 space-y-8 overflow-y-auto">
        <div>
          <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Aura Hubs</div>
          <nav className="space-y-1">
            <NavItem icon="💬" label="Companion" active={activeView === 'Companion'} onClick={() => setActiveView('Companion')} />
            <NavItem icon="🤖" label="Agent Hub" active={activeView === 'Agents'} onClick={() => setActiveView('Agents')} />
            <NavItem icon="⚒️" label="Training Forge" active={activeView === 'Training'} onClick={() => setActiveView('Training')} />
        ...
        <div>
          ...
          <StatRow label="Critical Assets" value={stats.critical} color="text-rose-600" />
        </div>
      </div>
      ...
      <div className="mt-auto pt-6 border-t border-slate-200">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="text-[10px] text-slate-500 uppercase tracking-tighter font-bold text-emerald-600">Enterprise Verified</div>
          </div>
        </div>
      </div>
    </aside>
...
const StatRow = ({ label, value, color = "text-slate-700" }: any) => (
  <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-tighter mb-2">
...
const NavItem = ({ icon, label, active, onClick }: any) => (
  ...
  >
    <span className="text-lg transition-all">{icon}</span>
